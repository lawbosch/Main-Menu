...
static const unsigned char memorymsg[]        PROGMEM = " memory bytes free.";
#ifdef ARDUINO
#ifdef ENABLE_EEPROM
static const unsigned char eeprommsg[]        PROGMEM = " EEProm bytes total.";
static const unsigned char eepromamsg[]       PROGMEM = " EEProm bytes free.";
#endif
#endif
...
{ 
  ...
  Serial.println("~                    ~");
  Serial.println("~                    ~");
  Serial.println("~   &&TinyBasic Plus!&  ~");
  Serial.println("~                    ~");
  Serial.println("~                    ~");
  Serial.println("~~~~~~~~~~~~~~~~~~~~~~");
  delay (5000);
  Serial.print("Loading OS");
  int Boot = 0;
  while (Boot < 10) { // Loop until the counter reaches 10
    Serial.print('.');
    delay(300);
    Boot++;
  }
...
/* only add if you want */
  ...
  //printmsg(memorymsg);   /* add this and the line above if you want to have memory displayed at boot */
//#ifdef ARDUINO
//#ifdef ENABLE_EEPROM
  // eprom size
  //printnum( E2END+1 );
  //printmsg( eeprommsg );   /* add this and the line above if you want to have memory displayed at boot */
//#endif /* ENABLE_EEPROM */
//#endif /* ARDUINO */
